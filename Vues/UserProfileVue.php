...
<center>
    ...
    public function afficherCover($user_id) {

        if(isset($_POST['disconnect'])) {
            session_destroy();
            echo '<script>window.location("http://localhost/project/Routers/LoginRoute.php")</script>';
        }
        $user_controller = new UserController();
        $user = $user_controller->getUserName($user_id);
        $user_name = $user[0]['user_name'];
        echo '<div id="id_info" data-userid="'.$user_id.'"></div>
<div class="profileCover" >
        <div class="userInfo">
            <img src="../assets/User/user.png">
            <h5 style="margin-top: 10px;color: white">'.$user_name.'</h5>

        </div>
        <img class="logoP" src="../assets/Accueil/logoWhite.png" >
    </div>';
    }

    public function afficherFavRecettes($user_id){//afficher les recettes favorites de l'utilisateur
        $user_controller = new UserController();
        $recettes_fav = $user_controller->getRecetteFav($user_id);
        echo '<button  class="btn btn btn-danger" id="Disconnect" style="margin: 20px" onclick="">Se déconnecter</button>';
        echo '<h2 id="titre_page" class="t3" style="margin-top: 20px">Mes recettes favorites</h2>';
...
';
        ...
        foreach ($recettes_fav as $recette) {
            ...
            <button value="'.$recette['id_Recette'].'" class="btn btn btn-danger" id="RmFromFav" style="margin: 20px" onclick="">Retirer de mes favoris</button>
            <div class="CardTitle">
    ...
    public function afficherMesRecettes($user_id){//afficher les recettes favorites de l'utilisateur
        $recette_c = new RecetteController();
        $recettes_fav = $recette_c->getMyRecette($user_id);
        echo '<h2 id="titre_page" class="t3" style="margin-top: 40px">Mes recettes</h2>';
        echo '
<div id="recetteListe" class="recettteContainer" style="margin-top: 20px;border-top: solid #757575 1px;padding-top: 50px">
';
        ...
        foreach ($recettes_fav as $recette) {
            ...
            data-portion ="'.$recette['portion'].'" 
            data-saison = "'.$recette['saison'].'"
            class="Card2" id="recette' . $recette['id_Recette'] . '">
            <div class="CardTitle">
                ' . $recette['titre_recette'] . '
            </div>
            <div class="CardImage">
