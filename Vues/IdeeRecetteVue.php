...
<center>
    ...
    public function afficherSelectMenu() {
        $ingred_controller = new IngredientsController();
        $ingredients = $ingred_controller->getIngredients();
        echo '<p style="color:#FF8243;font-size: 22px;margin-top: 20px;">Trouvez des recettes pour vos ingrédients de choix</p>
<select id="selectIngred" class="selectIngred form-select form-select-lg " aria-label="Default select example" style="width: 30%;font-size: 18px;display: flex">
<option value="0" selected>Selectionner des ingrédients</option>';
        foreach ($ingredients as $ingredient){
            echo '<option value="'.$ingredient[0].'">'.$ingredient[1].'</option>';
        }
        echo '</select>';
        echo '<ul class="list-group selectedIngred" id="selectedIngred"></ul>';
        echo '<button id="searchIngred" type="button" class="btn btn-primary">Rechercher</button>';
    }
